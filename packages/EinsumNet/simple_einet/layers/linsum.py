...
from packages.EinsumNet.simple_einet.layers.einsum import logsumexp
from packages.EinsumNet.simple_einet.sampling_utils import (
    index_one_hot,
...
class ConditionalLinsumLayer(LinsumLayer):
    """LinsumLayer where weights are generated from conditioning information."""
    ...
    def __init__(self, num_features: int, num_sums_in: int, num_sums_out: int,
        ...
        cond_fn_args.setdefault("out_dim", weight_dim)
        self.cond_fn = None
        if cond_fn_type is not None:
            self.cond_fn = eval(cond_fn_type)(**cond_fn_args)

    def forward(self, x: torch.Tensor, cond_input: torch.Tensor = None):
        ...
        prob = torch.logsumexp(prod_output + torch.log(weights), dim=2)
