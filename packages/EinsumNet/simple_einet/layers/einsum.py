...
from packages.EinsumNet.simple_einet.abstract_layers import AbstractSumLayer, logits_to_log_weights
from packages.EinsumNet.simple_einet.sampling_utils import SamplingContext, sample_categorical_differentiably, index_one_hot
...
class EinsumLayer(AbstractSumLayer):
    ...
    def get_context_weights(self, cond_input: Tensor):
        if cond_input is None or self.cond_fn is None:
            print("condition function is None")
            return None
        logits, context_embeds = self.cond_fn(cond_input, return_embedding=True)
...
class ConditionalEinsumLayer(EinsumLayer):
    """EinsumLayer where weights are generated from conditioning information."""

    def __init__(self, num_features: int, num_sums_in: int, num_sums_out: int,
                 num_repetitions: int = 1, dropout: float = 0.0,
                 cond_fn_type: str = None, cond_fn_args: dict = None):
        super().__init__(num_features=num_features, num_sums_in=num_sums_in,
                         num_sums_out=num_sums_out, num_repetitions=num_repetitions,
                         dropout=dropout)
        cond_fn_args = {} if cond_fn_args is None else cond_fn_args
        weight_dim = int(np.prod(self.weight_shape()))
        cond_fn_args = cond_fn_args.copy()
        cond_fn_args.out_dim = weight_dim
        self.cond_fn = None
        if cond_fn_type is not None:
    ...
    def forward(self, x: Tensor, cond_input: Tensor = None) -> Tensor:
        if cond_input is None or self.cond_fn is None:
        ...
        logits = self.cond_fn(cond_input)
        logits = logits.view(N, D_out, self.num_sums_out, self.num_repetitions, -1)
        weights = F.softmax(logits, dim=-1)
        weights = weights.view(N,*self.weight_shape())
        
        prob = torch.einsum("ndir,ndjr,ndorij->ndor", left_prob, right_prob, weights)
        prob = torch.log(prob) + left_prob_max + right_prob_max
