...
from packages.EinsumNet.simple_einet.layers.einsum import (
    EinsumLayer,
)
from packages.EinsumNet.simple_einet.layers.mixing import MixingLayer
from packages.EinsumNet.simple_einet.layers.factorized_leaf import FactorizedLeaf, ConditionalFactorizedLeaf
from packages.EinsumNet.simple_einet.layers.linsum import LinsumLayer
from packages.EinsumNet.simple_einet.sampling_utils import sampling_context, SamplingContext
from packages.EinsumNet.simple_einet.layers.sum import SumLayer
...
class EinetConfig:
    ...
    conditional_leaf: bool = False # Indicates whether to use connditional distribution as leafs
    conditional_sum: bool = False  # Indicates whether to obtain sum node parameters as conditional distributions
    sum_cond_fn_type: str = None  # Neural net module to map conditioning info to sum logits
    sum_cond_fn_args: Dict[str, Any] = field(default_factory=dict)  # Arguments for the conditioning module
    ...
    def assert_valid(self):
        ...
        if self.conditional_sum:
            assert self.sum_cond_fn_type is not None, "conditional_sum=True but sum_cond_fn_type was not provided"
...
class Einet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: torch.Tensor = None, cond_input=None) -> torch.Tensor:
        """
        ...
        ), f"Number of features in input ({x.shape[2]}) does not match number of features specified in config ({self.config.num_features})."

        # Apply leaf distributions (replace marginalization indicators with 0.0 first)
        x = self.leaf(x, marginalized_scopes) if not self.config.conditional_leaf else self.leaf(x, marginalized_scopes, cond_input) 
        # print("After leaf:", x)

        # Pass through intermediate layers
        x = self._forward_layers(x, cond_input)
    ...
    def get_context_weights(self, cond_input=None):
        for layer in self.layers:
            weights = layer.get_context_weights(cond_input)
        if weights is not None:
            return weights


    def _forward_layers(self, x, cond_input=None):
        """
        ...
        for layer in self.layers:
            x = layer(x) if not self.config.conditional_sum else layer(x, cond_input) 
            # if not self.config.conditional_sum:
            #     x = layer(x)
            #     context_embeds = None
            # else:
            #     x, context_embeds = layer(x, cond_input)    
        return x

    def posterior(self, x, cond_input=None) -> torch.Tensor:
        """
        ...
        # logp(x | y)
        ll_x_g_y = self(x, cond_input=cond_input)  # [N, C]
    ...
    def _build(self):
        ...
        for i in np.arange(start=1, stop=self.config.depth + 1):
            ...
            if self.config.layer_type == "einsum":
                layer_cls = EinsumLayer
                if self.config.conditional_sum:
                    from packages.EinsumNet.simple_einet.layers.einsum import ConditionalEinsumLayer
                    layer_cls = ConditionalEinsumLayer
                layer = layer_cls(
                    ...
                    cond_fn_type=self.config.sum_cond_fn_type,
                    cond_fn_args=self.config.sum_cond_fn_args,
                ) if self.config.conditional_sum else layer_cls(
                    num_features=in_features,
                    num_sums_in=_num_sums_in,
                    num_sums_out=_num_sums_out,
                    num_repetitions=self.config.num_repetitions,
                ...
                layer_cls = LinsumLayer
                if self.config.conditional_sum:
                    from packages.EinsumNet.simple_einet.layers.linsum import ConditionalLinsumLayer
                    layer_cls = ConditionalLinsumLayer
                layer = layer_cls(
                    num_features=in_features,
                    num_sums_in=_num_sums_in,
                    num_sums_out=_num_sums_out,
    ...
    def _build_input_distribution(self, num_features_out: int):
        ...
        return FactorizedLeaf(
            ...
            num_repetitions=self.config.num_repetitions,
            base_leaf=base_leaf,
        ) if not self.config.conditional_leaf else ConditionalFactorizedLeaf(
            num_features=base_leaf.out_features,
            num_features_out=num_features_out,
            num_repetitions=self.config.num_repetitions,
            base_leaf=base_leaf,
