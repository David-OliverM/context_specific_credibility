...
def add_env_noise(waveform, sample_rate, intensity, noise_dir=''):
    ...
    eps = 1e-8
    noise_abs = noise.abs()
    clean_abs = waveform.abs()

    corruption_mask = noise_abs / (noise_abs + clean_abs + eps)

    return waveform_noisy, corruption_mask
...
def snow_visual(x, severity=5):
    ...
    snow_layer = cv2.imdecode(
        np.frombuffer(snow_motion.make_blob(), np.uint8),
        cv2.IMREAD_UNCHANGED
    ) / 255.0
    snow_layer = snow_layer[..., np.newaxis]

    snow_layer_rot = np.rot90(snow_layer, k=2)
    corruption_mask = np.clip(snow_layer + snow_layer_rot, 0, 1)

    gray = cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(h, w, 1)
    x = c[6] * x + (1 - c[6]) * np.maximum(x, gray * 1.5 + 0.5)

    x = np.clip(x + corruption_mask, 0, 1) * 255
    x = Image.fromarray(x.astype(np.uint8))
...
def snow_audio(waveform, intensity=5):    
    snow_dir = '/home/pxt220000/Projects/CS_Credibility/packages/AVROBUSTBENCH/corruptions/audio_corruptions/snow'
...
def frost_visual(x, severity=5):
    ...
    frost_dir = '/home/pxt220000/Projects/CS_Credibility/packages/AVROBUSTBENCH/corruptions/visual_corruptions'
    filename = [
        f"{frost_dir}/frost1.png",
        f"{frost_dir}/frost2.png",
        f"{frost_dir}/frost3.png",
        f"{frost_dir}/frost4.jpg",
        f"{frost_dir}/frost5.jpg",
        f"{frost_dir}/frost6.jpg"
    ][idx]

    frost = cv2.imread(filename)
    frost = frost[..., [2, 1, 0]]  # BGR → RGB

    x_np = np.array(x, dtype=np.float32)
    h, w = x_np.shape[:2]

    # Random crop
    x_start = np.random.randint(0, frost.shape[0] - h)
    y_start = np.random.randint(0, frost.shape[1] - w)
    frost = frost[x_start:x_start + h, y_start:y_start + w].astype(np.float32)

    # Apply corruption
    x_corrupt = np.clip(c[0] * x_np + c[1] * frost, 0, 255)

    # ---- Corruption mask ----
    eps = 1e-6
    frost_contrib = c[1] * frost
    clean_contrib = c[0] * x_np

    corruption_mask = frost_contrib / (clean_contrib + frost_contrib + eps)
    corruption_mask = np.clip(corruption_mask / 255.0, 0, 1)

    x_corrupt = Image.fromarray(x_corrupt.astype(np.uint8))

    return x_corrupt, corruption_mask


def frost_audio(waveform, intensity=5):
    frost_dir = '/home/pxt220000/Projects/CS_Credibility/packages/AVROBUSTBENCH/corruptions/audio_corruptions/frost'
...
def rain_visual(x, severity=5):
    ...
    # --- Generate rain layer ---
    rain_layer = np.random.normal(loc=c[0], scale=c[1], size=(h, w))
    ...
    rain_np = cv2.imdecode(
        np.frombuffer(wand_rain.make_blob(), np.uint8),
        cv2.IMREAD_UNCHANGED
    )

    if rain_np is None:
        rain_np = rain_layer[..., 0]
    else:
        if rain_np.ndim == 3:
            rain_np = rain_np[..., 0]
    ...
    # --- Colorize rain ---
    rain_color = np.stack(
        [rain_np * 0.5, rain_np * 0.9, rain_np * 1.2],
        axis=-1
    )
    rain_color = np.clip(rain_color, 0, 1)

    rain_color_rot = np.rot90(rain_color, k=2)

    # ---- Corruption mask ----
    corruption_mask = np.clip(rain_color + rain_color_rot, 0, 1)

    # --- Apply grayscale mixing ---
    alpha = c[6]
    gray_x = cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(h, w, 1)
    x = alpha * x + (1 - alpha) * np.maximum(x, gray_x * 1.3 + 0.3)

    # --- Apply rain ---
    final = np.clip(x + corruption_mask, 0, 1) * 255.0
...
def rain_audio(waveform, intensity=5):
    rain_dir = '/home/pxt220000/Projects/CS_Credibility/packages/AVROBUSTBENCH/corruptions/audio_corruptions/rain'
